...
def process_one(chart_dir: Path, rel_path: str, cache_dir: Path = CACHE_DIR, sample_rate = 22050, n_fft=1024, hop_length = 256, n_mels = 80) -> Path:
...
def main(charts_dir = CHARTS_DIR, cache_dir = CACHE_DIR, sample_rate = 22050, n_fft=1024, hop_length = 256, n_mels = 80):
    ...
    for chart_dir, _dirs, files in charts_dir.walk():
        ...
        try:
            out = process_one(chart_dir, str(rel), cache_dir, sample_rate, n_fft, hop_length, n_mels)
            found += 1
